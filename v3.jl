...
function make_figure()
    ...
    live_plot = heatmap(board, colormap = :hotpink, colorrange = (0, 1))
...
function next_generation!(board)
    ...
    new_board = similar(board) # Similar(board) vytvori nove pole se stejnym typem a rozmery jako board
...
function gol(;gens::Int, delay::Float64)
    if start == true
        ...
        return println("Simulation not started. Set start to true to run the simulation.")
    end
end 
make_figure()
